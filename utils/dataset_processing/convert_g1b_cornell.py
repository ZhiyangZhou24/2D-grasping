    ...
    for camera in ['realsense']:
        ...
        for scene_id in range(scene_start, scene_end):
            for ann_id in range(256):
                ...
                rect_grasps: RectGraspGroup = g.loadGrasp(sceneId=scene_id, camera=camera, fric_coef_thresh = 0.4, annId=ann_id, format='rect')
                # sort them
                rect_grasps = rect_grasps.sort_by_score()
                n = (int) (len(rect_grasps) * 0.5) # only use top-5% grasps
