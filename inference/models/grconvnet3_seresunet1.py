...
class conv_att(nn.Module):
    ...
    def __init__(self, in_ch, out_ch,att_type = 'use_eca', reduc_ratio=3):
...
class down(nn.Module):
    def __init__(self, in_ch, out_ch,att_type='use_eca'):
        super(down, self).__init__()
        self.mpconv = nn.Sequential(
            nn.MaxPool2d(kernel_size = 2),
            conv_att(in_ch, out_ch,att_type=att_type)
        )
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch,att_type='use_eca',upsample_type='use_bilinear'):
        ...
        self.conv = conv_att(in_ch, out_ch,att_type = att_type)
...
class GenerativeResnet(GraspModel):
    ...
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, att = 'use_eca',upsamp='use_bilinear', dropout=False, prob=0.0):
        ...
        self.inconv = inconv(input_channels,channel_size * 2)

        self.down1 = down(channel_size * 2, channel_size * 4, att_type=att)
        self.down2 = down(channel_size * 4, channel_size * 8, att_type=att)
        self.down3 = down(channel_size * 8, channel_size * 16, att_type=att)
        self.down4 = down(channel_size * 16, channel_size * 16, att_type=att)
        self.up1 = up(channel_size * 16 * 2, channel_size * 8, att_type=att)
        self.up2 = up(channel_size * 8 * 2, channel_size * 4, att_type=att)
        self.up3 = up(channel_size * 4 * 2, channel_size * 2, att_type=att)
        self.up4 = up(channel_size * 2 * 2, channel_size * 1, att_type=att)
