...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=[3, 3], e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        self.cv1 = ConvBNLayer(c1, c_, 3)
        self.cv2 = ConvBNLayer(c_, c2, 3, groups=g)
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5,act="mish"):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        self.cv1 = ConvBNLayer(c1, 2 * self.c, 1, 1,act=act)
        self.cv2 = ConvBNLayer((2 + n) * self.c, c2, 1,act=act)  # optional act=FReLU(c2)
